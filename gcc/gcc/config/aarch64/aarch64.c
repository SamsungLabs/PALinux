...
aarch64_return_address_signing_enabled (void)
{
  ...
  return (aarch64_ra_sign_scope == AARCH64_FUNCTION_ALL
	  ...
	  || aarch64_return_address_signing_for_ssp()
	  || aarch64_return_address_signing_for_alt());
}

bool
aarch64_return_address_signing_for_ssp (void)
{
  return (aarch64_ra_sign_scope == AARCH64_FUNCTION_SSP
	  && crtl->stack_protect_guard);
}

bool
aarch64_return_address_signing_for_alt (void)
{
  return (aarch64_ra_sign_scope == AARCH64_FUNCTION_ALT
	  && crtl->stack_protect_guard);
}
  ...
  if (aarch64_return_address_signing_enabled ())
    {
      if (aarch64_return_address_signing_for_alt())
        {
          insn = emit_insn (gen_alternative_pauth ());
        }
