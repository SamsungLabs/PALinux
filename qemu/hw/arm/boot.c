...
static int fdt_set_seed(void *fdt, const char* prop, size_t size) {
    int rc;
    uint8_t key[size];

    qemu_guest_getrandom_nofail(key, size);
    rc = qemu_fdt_setprop(fdt, "/chosen", prop, key, size);
    if (rc < 0) {
        fprintf(stderr, "couldn't set /chosen/%s", prop);
    }
    return rc;
}
                 ...
                 hwaddr addr_limit, AddressSpace *as)
{
    ...
    /* putting random seeds for early setup routines */
    if (fdt_set_seed(fdt, "kaslr-seed", 8) < 0)
        goto fail;
    if (fdt_set_seed(fdt, "pauth-seed", 32) < 0)
        goto fail;
